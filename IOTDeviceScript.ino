...
const char ssid[] = "HARDOMO 2.4G"; // TODO cambiar por el nombre de la red WiFi
...
const char MQTT_TOPIC_PUB[] = "colombia/risaralda/pereira/" USER "/out"; //TODO Reemplace el valor por el tópico de publicación que le corresponde.
// El tópico de suscripción debe tener estructura: <país>/<estado>/<ciudad>/<usuario>/in
const char MQTT_TOPIC_SUB[] = "colombia/risaralda/pereira/" USER "/in"; //TODO Reemplace el valor por el tópico de suscripción que le corresponde.

// Declaración de variables globales

// Timestamp de la fecha actual.
time_t now;
// Tiempo de la última medición de temperatura y humedad
long long int measureTime = millis();
// Tiempo de la última medición del nivel de batería
long long int batteryMeasureTime = millis();
...
/**
 * Publica un valor de nivel de batería
 */
void sendBatteryData(float batt) {
  String data = "{";
  data += "\"bateria\": "+ String(batt, 1);
  data += "}";
  char payload[data.length()+1];
  data.toCharArray(payload,data.length()+1);
  
  client.publish(MQTT_TOPIC_PUB, payload);
  Serial.println("Nivel de batería enviado");
}
...
String checkAlert() {
  ...
  if (alert.length() != 0) {
    ...
    if(alert.indexOf("CHARGE THE BATTERY NOW")>=0){
